...
Optionally, using the --delete flag, this script can also be used to delete a transfer object.  Note that this deletes the transfer records, not any exported transfer folders on disk generated by the process_transfers script. To delete transfer record(s) that are not flagged for processing, do the following:

./touch_transfers REPOID --delete

To delete transfer record(s) that are flagged for processing:

./touch_transfers REPOID --delete --unset

You can also specify a transfer object for deletion that is not flagged for processing:

./touch_transfers REPOID --dataobj=1 --delete

If the transfer object(s) that you want to delete are set for processing (i.e., "is_dirty" is TRUE), do the following to delete:

./touch_transfers REPOID --dataobj=1 --unset --delete
