...
sub output_dataobj
{
	...
	foreach my $doc ( @docs )
	{
		...
		foreach my $file ( @{$doc->get_value( "files" )} )
		{
			...
			if (! defined ($h)) {
				push @results, $self->_log("WARNING - Checksum MISSING - reshashing new MD5 for file ", "'$file_path/$filename'", 2);
				$file->update_md5;
				$file->commit;
	...
	foreach my $file_path ( @file_paths )
	{
		...
		if ( !defined( $hash_cache{ $file_path } )) {
			...
			#mismatch
			$ok = 0;
			push @results, $self->_log("ERROR - checksum MISMATCH ", "$file_path");	
		}
